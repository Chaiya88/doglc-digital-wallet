...
function securityMiddleware(request, env2) {
  const url = new URL(request.url);
  const path = url.pathname;
  if (path.includes("..") || path.includes("%2e%2e") || path.includes("..%2f") || path.includes("..\\") || path.includes("%2e%2e%2f") || path.includes("%5c%2e%2e") || path.includes("..%5c")) {
    console.log("\u{1F6A8} Path traversal attempt detected:", path);
  ...
  if (requests.length >= maxRequests) {
    console.log("\u{1F6A8} Rate limit exceeded for IP:", clientIP);
    return new Response(JSON.stringify({
...
var secure_api_default = {
  async fetch(request, env2, ctx) {
    const url = new URL(request.url);
    const path = url.pathname;
    const securityCheck = securityMiddleware(request, env2);
    if (securityCheck) {
      return addSecurityHeaders(securityCheck);
    }
    if (request.method === "OPTIONS") {
      const response2 = new Response(null, { status: 200 });
...
// .wrangler/tmp/bundle-1FO92Q/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
  middleware_miniflare3_json_error_default
];
var middleware_insertion_facade_default = secure_api_default;
...
// .wrangler/tmp/bundle-1FO92Q/middleware-loader.entry.ts
...
//# sourceMappingURL=secure-api.js.map
